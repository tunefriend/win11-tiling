#!/bin/bash
# Build win11-window-tiling-kde .deb for Debian KDE Plasma.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
PKGROOT="$ROOT/pkgroot"
PKG="win11-window-tiling-kde_1.0.0_all.deb"

chmod 755 "$PKGROOT/DEBIAN/postinst" "$PKGROOT/DEBIAN/prerm"
chmod 755 "$PKGROOT/usr/bin/"*

# Ensure control file ends with newline (required by dpkg)
...
echo ""
echo "Install on Debian KDE Plasma:"
