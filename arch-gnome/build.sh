#!/bin/bash
# Build win11-window-tiling-gnome package for Arch Linux.
# Run on Arch: ./build.sh
# Cross-build: requires makepkg (and fakeroot recommended).

set -euo pipefail

ROOT="$(cd "$(dirname "$0")" && pwd)"
PKGNAME="win11-window-tiling-gnome"
VERSION="1.0.0"
RELEASE="1"
...
if command -v makepkg >/dev/null 2>&1; then
    ...
    if [ -n "$PKG" ] && [ -f "$PKG" ]; then
        ...
        echo ""
        echo "Install on Arch Linux (GNOME):"
